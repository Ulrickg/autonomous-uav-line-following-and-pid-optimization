...
assignin('base', 'kd_yaw_angle',   kd_yaw_angle);
...
mdl = 'hoverSim_pid';
open_system(mdl);
kd_var = optimizableVariable('kd_pitch_angle', [0, 0.10], 'Type', 'real');
results = bayesopt(@objectivePitchAngleD, kd_var, ...
'MaxObjectiveEvaluations',  30, ...
'IsObjectiveDeterministic', true, ...
'AcquisitionFunctionName',  'expected-improvement-plus', ...
'Verbose',                   1, ...
'PlotFcn', {@plotObjectiveModel, @plotMinObjective});
best_Kd = results.XAtMinObjective.kd_pitch_angle;
fprintf('\nBeste kd_pitch_angle: %.4f\n', best_Kd);
kd_pitch_angle = best_Kd;
save('tuned_pitch_angle_d.mat', 'kd_pitch_angle');
disp('Lagret til tuned_pitch_angle_d.mat');